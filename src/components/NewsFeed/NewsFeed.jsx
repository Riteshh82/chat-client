...
const FALLBACK_ARTICLES = [
  {
    ...
    description:
      "As AI systems become more capable, researchers and ethicists grapple with questions about autonomy, privacy, and what it means to delegate decisions to machines.",
  ...
  },
  {
    ...
    description:
      "From Linux servers to React frontends, open source powers nearly everything online — yet most of its maintainers are volunteers working in their spare time.",
  ...
  },
  {
    ...
    description:
      "Both protocols enable real-time communication between client and server, but they serve very different use cases. Here's a deep-dive comparison.",
  ...
  },
  {
    ...
    description:
      "With over 60% of web traffic coming from mobile devices, designing for smaller screens first isn't just good practice — it's essential for survival.",
  ...
  },
  {
    ...
    description:
      "A practical guide to configuring indexes, aggregation pipelines, and Atlas Search for applications that need to grow without pain.",
  ...
  },
  {
    ...
    description:
      "Variable reward schedules, social proof, and the dopamine loop — chat apps exploit human psychology in ways we're only beginning to understand.",
    source: { name: "Wired" },
  ...
  },
  {
    ...
    description:
      "From clustering and caching to efficient event loop management, these strategies will help you squeeze maximum throughput from your Node servers.",
  ...
  },
  {
    ...
    description:
      "The React team has been busy. From the new compiler to server components reaching stability, here's everything you need to know about the latest release.",
...
export default function NewsFeed({ onJoin }) {
    ...
    <div className="news-feed">
      ...
      <div className="news-tabs">
        {["For You", "Tech", "Science", "Design", "World"].map((tab, i) => (
          <button
            key={tab}
            className={`news-tab ${i === 0 ? "news-tab--active" : ""}`}
          >
            {tab}
          </button>
        ))}
                  ...
                  <div className="news-featured__dots">
                    {articles.slice(0, 3).map((_, i) => (
                      <span
                        key={i}
                        className={`news-dot ${
                          i === featuredIndex ? "news-dot--active" : ""
                        }`}
      ...
      {showModal && (
        <JoinModal onJoin={onJoin} onClose={() => setShowModal(false)} />
      )}
    </div>
  );
}
...
const ArticleCard = React.memo(function ArticleCard({
  ...
  return (
    <article className="article-card" style={{ animationDelay: `${delay}ms` }}>
      <div
